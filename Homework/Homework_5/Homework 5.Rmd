...
surrogate1 <- X_s[, 1]
surrogate2 <- (X_s[, 7] + X_s[, 8]) - X_s[, 2]
surrogate3 <- (X_s[, 1] + X_s[, 2] + X_s[, 3]) - X_s[, 8]
surrogate4 <- (X_s[, 6]+ X_s[, 7]) + X_s[, 8]
surrogate5 <- (X_s[, 7] + X_s[, 8])
...
ols_surrogate_model <- lm(housing_train$medianValue ~ ., data = surrogate_data)

print(vif(ols_surrogate_model))

print('Sorry, I have no time to finish after work')
```
...
SORRY I HAVE NOT TIME TO FINISH
